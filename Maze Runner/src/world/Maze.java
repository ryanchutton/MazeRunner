...
public class Maze extends JFrame {
	...
	public Maze(String str) {
		 ...
		 *           revalidate(); repaint();
		 * 
		 *           //Player movement if(key == KeyEvent.VK_W){ p.moveUp(); }
		 *           if(key == KeyEvent.VK_A){ p.moveLeft(); } if(key ==
		 *           KeyEvent.VK_S){ p.moveDown(); } if(key == KeyEvent.VK_D){
		 *           p.moveRight(); }
		 * 
		 *           if(p.x == columns-1 && p.y == endLevelLoc){
		 *           JOptionPane.showMessageDialog(null, "Congratulations,
		 *           you've beaten the level!", "End Game",
		 *           JOptionPane.INFORMATION_MESSAGE); dispose(); new
		 *           menu.MainMenu(); } }
		 * 
